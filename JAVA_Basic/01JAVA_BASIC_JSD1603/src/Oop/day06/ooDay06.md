...
1.多态:
  1)多态的意义:
    1.1)同一类型的引用指向不同的对象时(父亲、叔叔、伯伯实现爷爷)，同一方法(功能)有不同的实现 ——对象决定方法的实现！
        ----行为的多态:cut(),run()
    1.2)同一个对象被造型为不同的类型时(孙子继承父辈、孙子继承爷爷)，有不同的功能(方法)  ——类型决定能有什么功能(方法)！
        ----对象的多态:我,水
  2)向上造型:
    2.1)父类型的引用指向子类的对象
    2.2)能造型成的类型: 
...
2.1.成员内部类:**扩充的普通成员属性**(比普通属性功能强大)  
  ...
  4)内部类可以直接访问外部类的成员(所有变量、方法)  
    4.1)访问外部类非静态:  
        ...
        b.直接访问: 属性、方法(如果内部类中有与外部类相同名称的属性、方法时，优先调用内部类的， age、this.age)  
...
4.局部内部类：**扩充类方法的功能**  
    1) 在**外部类的方法中**添加一个内部类，通过自动调用内部类中方法达到扩充外部类方法的功能  

5.**匿名内部类**: 一次性(临时实现)用一次父类或接口的**功能(方法)**   
...
那么该局部变量相当于自动使用了final修饰。）   
    ...
    9)**匿名内部类的使用场景**: 当类或接口类型作为参数(实参)传递时，可以直接使用匿名内部类方式创建对应的对象!  
